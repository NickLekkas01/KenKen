...
class Sudoku(CSP):
    ...
    def __init__(self, grid):
        ...
        print("Sudoku:Domains:",domains)
        print("Sudoku:Neighbors:",self.neighbors)
...
hard_ken = [[2, '-', 0, 8], [7, '-', 1, 2], [1, '-', 3, 4], [2, '-', 5, 6], [120, '*', 7, 15, 23], [96, '*', 9, 10, 18], [35, '*', 11, 12, 19], [2, '/', 13, 21], [3, '/', 14, 22], [1, '-', 16, 24], [2, '/', 17, 25], [15, '+', 20, 28], [3, '-', 26, 27], [24, '*', 29, 30, 31, 38], [8, '+', 32, 33], [1, '-', 34, 35], [10, '*', 36, 44], [336, '*', 37, 45, 46], [7, '-', 39, 47], [105, '*', 40, 41, 49], [1, '-', 42, 50], [5, '-', 43, 51], [1, '-', 48, 56], [2, '-', 52, 60], [1, '-', 53, 61], [504, '*', 54, 55, 62, 63], [16, '+', 57, 58, 59]]
medium_ken = [[11, '+', 0, 6], [2, '/', 1, 2], [3, '-', 7, 8], [20, '*', 3, 9], [6, '*', 4, 5, 11, 17], [3, '/', 10, 16], [240, '*', 12, 13, 18, 19], [6, '*', 14, 15], [6, '*', 20, 26], [7, '+', 21, 27, 28], [30, '*', 22, 23], [6, '*', 24, 25], [8, '+', 30, 31, 32], [2, '/', 33, 34], [9, '+', 29, 35]]
easy_ken = [[1,'-',0,3],[1,'-',1,2],[3,'/',6],[2,'/',4,7],[3,'/',5,8]]
class KenKen(CSP):
    times = 0
    """A KenKen problem.
    The box grid is a 6x6 array of boxes.
    Each cell holds a digit in 1..6. In each box, all digits are
    different; the same for each row and column as a 6x6 grid.
    """
    # The method that prints all  
    # possible strings of length k. 
    # It is mainly a wrapper over  
    # recursive function printAllKLengthRec() 
    def AllKLength(set, item, k):
        """
        :param set: All appropriate numbers [1:n]
        :param item: the current segment
        :param k: number of variables in segment
        :return: all the possible results for the segment
        """
        n = len(set)
        """
        U is the list with the values we will use for the domain
        """
        U = list()
        U = KenKen.AllKLengthRec(set, item, U, [], n, k)
        
        return U
    # The main recursive method 
    # to print all possible  
    # strings of length k 
    def AllKLengthRec(set, item, U, prefix, n, k):
        """
        :param set: All appropriate numbers [1:n]
        :param item: the current segment
        :param U: the list for the possible results
        :param prefix: list to save the numbers for the result
        :param n: number of appropriate numbers
        :param k: number of variables in segment
        :return: all the possible results for the segment
        """
        ...
        if  (k == 0) :
            ...
            """Find the result of the numbers after the mathematical operation"""
            total = temp[0]
            for index in list(range(1,len(temp))):
                if item[1]=='+':
                    total+=temp[index]
                elif item[1]=='-':
                    total-=temp[index]
        ...
        for i in range(n): 
            ...
            U = KenKen.AllKLengthRec(set, item, U, newPrefix, n, k - 1)
        return U
    ...
    def __init__(self, grid, n):
        ...
        def kenken_constraint(A, a, B, b):
            """A constraint saying two neighboring variables must differ in value."""
            #Number of times the constrain function is called
            KenKen.times += 1
            """
            If it is the same variable then we need to have the same value
            """
            if A == B:
                return a == b
            """
            For every box of our segment(A) we check if it is in the same line or row of the boxes of the other segment(B)
            If it is then they must have different values
            If there is at least one combination with the save value and it is in the same line or row then we return False
            """
            k1 = grid[A]
            k2 = grid[B]
            for i in list(range(2,len(k1))):
                for j in list(range(2,len(k2))):
                    if (k1[i] // n == k2[j] // n) or (k1[i] % n == k2[j] % n):
                        if a[i-2] == b[j-2]:
                            return False
            return True

        """We have as many variables as the segments"""
        var = list(range(len(grid)))
        """
        We insert as neighbors the segments in which at least one box
        of those segments is in the same line or row with a box of our segment
        """
        neighbors = dict()
        for counter in list(range(len(grid))):
            i = grid[counter]
            for j in i[2:]:
                #lines
                for counter2 in list(range(len(grid))):
                    k = grid[counter2]
                    if i==k:
                        continue
                    for l in k[2:]:
                        if l % n == j % n or l//n == j//n:
                            if counter not in neighbors:
                                neighbors[counter] = [counter2]
                            else:
                                if counter2 not in neighbors[counter]:
                                    neighbors[counter].append(counter2)

        print("Variables:", var)
        print("Neighbors:", neighbors)
        """
        For the domain we have all the possible solutions for each segment using brute force
        """
        domains = dict()
        for index in list(range(len(grid))):
            i = grid[index]
            U = KenKen.AllKLength(list(range(1,n+1)), i, len(i)-2)
            """Save the possible solutions to the domain"""
            domains[index] = U
        print("Domains:", domains)
        print("Grid:", grid)
        CSP.__init__(self, var, domains, neighbors, kenken_constraint)
...
if __name__ == "__main__":
    ...
    """
    n = 8
    game = KenKen(hard_ken, n)
    start_time = time.time()
    assignment = backtracking_search(game)
    #assignment = backtracking_search(game, select_unassigned_variable=mrv)
    #assignment = backtracking_search(game, inference=forward_checking)
    #assignment = backtracking_search(game, select_unassigned_variable=mrv, inference=forward_checking)
    #assignment = backtracking_search(game, inference=mac)
    #assignment = min_conflicts(game)

    print("Answer:", assignment)
    print("Time:", (time.time() - start_time))
    print("Constrain Function times:",game.times)